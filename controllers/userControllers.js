...
const create = async (req, res) => {
    ...
    await check('name').notEmpty().withMessage('El nombre no puede ir vacío <img src="/assets/error.png" alt="Error" style="width: 20px; height: 20px; vertical-align: middle; margin-left: 177px; display: inline-block;" />').run(req);
    ...
    await check('password').isLength({ min: 6 }).withMessage('La contraseña debe tener al menos 6 caracteres. <img src="/assets/error.png" alt="Error" style="width: 20px; height: 20px; vertical-align: middle; margin-left: 16px; display: inline-block;" />').run(req);
    ...
    await check('birthDate').isISO8601().withMessage('La fecha de nacimiento debe ser válida <img src="/assets/error.png" alt="Error" style="width: 20px; height: 20px; vertical-align: middle; margin-left: 90px; display: inline-block;" />').bail().custom((value) => {
    ...
    .withMessage('Debe ser mayor de edad para registrarse. <img src="/assets/error.png" alt="Error" style="width: 20px; height: 20px; vertical-align: middle; margin-left: 10px; display: inline-block;" />')
    .run(req);
    ...
    res.render('template/message', {
        page: 'Cuenta creada',
        msg: `Se ha enviado un email de confirmación a: ${req.body.email}, por favor, ingrese al siguiente enlace`
    });
};
